...
class FileStorage(BaseStorage):
    ...
    async def init_db(self) -> None:
        ...
        if not self.token_file.exists():
            await self._write(
                self.token_file,
                orjson.dumps(
                    {"ssoNormal": {}, "ssoSuper": {}}, option=orjson.OPT_INDENT_2
                ).decode(),
            )
    ...
    async def _load_json(
        self, path: Path, default: Dict, lock: asyncio.Lock
    ) -> Dict[str, Any]:
        """加载JSON"""
        try:
            async with lock:
    ...
    async def _save_json(self, path: Path, data: Dict, lock: asyncio.Lock) -> None:
        ...
        try:
            async with lock:
                await self._write(
                    path, orjson.dumps(data, option=orjson.OPT_INDENT_2).decode()
                )
        except Exception as e:
    ...
    async def load_tokens(self) -> Dict[str, Any]:
        ...
        data = await self._load_json(
            self.token_file, {"ssoNormal": {}, "ssoSuper": {}}, self._token_lock
        )
    ...
    async def load_config(self) -> Dict[str, Any]:
        """加载配置"""
        return await self._load_toml(
            self.config_file, {"global": {}, "grok": {}}, self._config_lock
        )
...
class MysqlStorage(BaseStorage):
    ...
    async def init_db(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"[Storage] MySQL: {parsed['user']}@{parsed['host']}:{parsed['port']}/{parsed['db']}"
            )
    ...
    async def _sync_data(self) -> None:
        ...
        try:
            for table, key in [
                ("grok_tokens", "ssoNormal"),
                ("grok_settings", "global"),
            ]:
                ...
                if data:
                    ...
                    file_data = await (
                        self._file.load_tokens()
                        if table == "grok_tokens"
                        else self._file.load_config()
                    )
                    if (
                        file_data.get(key)
                        or file_data.get("sso")
                        or (table == "grok_tokens" and file_data.get("ssoSuper"))
                    ):
    ...
    async def _load_db(self, table: str) -> Optional[Dict]:
        ...
        try:
            async with self._pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    await cursor.execute(
                        f"SELECT data FROM {table} ORDER BY id DESC LIMIT 1"
                    )
            ...
            async with self._pool.acquire() as conn:
                async with conn.cursor() as cursor:
                    ...
                    if result:
                        ...
                        await cursor.execute(
                            f"INSERT INTO {table} (data) VALUES (%s)", (json_data,)
                        )
        except Exception as e:
...
class RedisStorage(BaseStorage):
    ...
    async def init_db(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"[Storage] Redis: {parsed.hostname}:{parsed.port or 6379}/{db}"
            )
            ...
            for key, file_func, key_name in [
                ...
                if data:
                    ...
                    if (
                        ...
                        or file_data.get("sso")
                        or (key == "grok:tokens" and file_data.get("ssoSuper"))
                    ):
...
class StorageManager:
    ...
    async def init(self) -> None:
        ...
        self._storage = (
            storage_class(url, data_dir) if mode != "file" else storage_class(data_dir)
        )

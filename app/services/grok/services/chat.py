                    ...
                    # 音频类型
                        ...
                        if is_video:
                            raise ValueError(
                                ...
                                "For image-to-video generation, use 'image_url' type instead."
                            )
...
class ChatRequestBuilder:
    ...
    def build_payload(
        ...
        """
        temporary = get_config("chat.temporary", True)
        ...
        payload["disableMemory"] = disable_memory
        payload["isReasoning"] = bool(think)
...
class GrokChatService:
    ...
    def __init__(self, proxy: str = None):
        # 代理将在请求时动态获取
        self._static_proxy = proxy or get_config("network.base_proxy_url", "")
    ...
    async def _get_proxy(self) -> str:
        ...
        # 优先使用代理池
        dynamic_proxy = await proxy_pool.get_proxy()
        if dynamic_proxy:
    ...
    async def chat(
        ...
        """
        if stream is None:
            stream = get_config("chat.stream", True)
        ...
        timeout = get_config("network.timeout", TIMEOUT)
...
class ChatService:
    ...
    async def _record_stats(
        ...
        try:
            ...
            await request_logger.add_log(
                ...
                key_name=key_name,
                token_suffix=token[-8:] if token and len(token) >= 8 else "",
                error=error,
            )
        except Exception as e:
    ...
    async def _wrap_stream(
        ...
        try:
            ...
            keepalive = 15.0
        stream = with_keepalive(stream, keepalive, ping_message=": ping\n\n")
        try:

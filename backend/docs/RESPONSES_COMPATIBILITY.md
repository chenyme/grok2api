...
When `GROK2API_STRIP_EXTERNAL_TOOLS` is set to `1`/`true`/`yes`/`on`, the Build adapter additionally strips client-side external tools that Grok Build cannot host, including `computer_use_preview` and `web_search*` declarations. Function-backed tools such as `function`, `apply_patch`, `tool_search` and shell remain available after normalization. Matching `tool_choice` is removed when no tools remain or when it points at a stripped external tool; unknown external tool history items are replaced with developer boundary messages. Compatibility warnings may include `external_tools_omitted` and `external_tool_history_omitted`, and a developer boundary is prepended so the model continues without those client-side tools.

The adapter also normalizes multi-turn history for Build: assistant `message` items with `output_text` are rewritten as plain `input_text` messages; `function_call_output` is string-encoded; and string-form `shell_call_output` is upgraded into structured stdout/stderr/outcome blocks.

发生协议降级时，响应 Header `X-Grok2API-Compatibility-Warnings` 会返回稳定的逗号分隔代码，例如 `web_search_controls_downgraded`、`web_search_disabled_no_external_access`、`web_search_tool_choice_disabled`、`external_tools_omitted`、`external_tool_history_omitted`、`legacy_local_shell_upgraded`、`apply_patch_emulated`、`additional_tools_position_approximated`。客户端可记录这些代码进行审计，不包含提示词、工具参数或凭据。
